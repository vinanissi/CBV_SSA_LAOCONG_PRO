...
**Single entry point for TASK_CENTER.** Task belongs to DON_VI; users shared system-wide. GAS is the workflow engine.
...
|-----------|----------------|
| Task belongs to DON_VI | TASK_MAIN.DON_VI_ID → DON_VI |
...
| **04_APPSHEET/APPSHEET_TASK_REF_MAP.md** | DON_VI_ID, OWNER_ID, REPORTER_ID ref mapping |
| **04_APPSHEET/APPSHEET_TASK_VIEW_MAP.md** | TASK_LIST, TASK_DETAIL, inlines |

**Refs:** ACTIVE_DON_VI (DON_VI), ACTIVE_USERS (USER_DIRECTORY). Allow Adds = OFF on all operational refs.
...
|-------|-----|--------|-------|
| TASK_MAIN | ID | — | DON_VI_ID optional, OWNER_ID required |
...
| 4 | `runTaskTests()` | Validates task workflow (requires DON_VI and active user) |
