...
| Attribute | V0 rule |
|-----------|---------|
| **Identity** | Conceptual; **Case Key** — `OCMS_CASE_KEY_AUTHORITY.md` (derive from primary anchor) |
...
| **Case Key** | **Defined** — `OCMS_CASE_KEY_AUTHORITY.md` (phase 02D); logical read-session id, not persisted |
...
## 13. Related model — Case Relation extension (V0D)

Case links to business entities (conceptual):

- `00_SYSTEM_BRAIN/OCMS/OCMS_CASE_RELATION_MODEL.md`
- `00_SYSTEM_BRAIN/002_DECISIONS/ADR_OCMS_CASE_RELATION_ADDENDUM.md`

Complete OCMS frame:

```text
CASE → CASE TYPE | LIFECYCLE | RELATION(S) | RESPONSIBILITY | WORK/STEPS | MEMORY | RESULT | MODULE PROJECTION
```

This section does not alter §3–§12 definitions above.

---

## 14. Related contract — OCMS Read Model Contract (V0E)

Read-only **`CaseReadModel`** JSON contract for OCMS_03+ (derive from AS-IS runtimes — no Case store):

- `00_SYSTEM_BRAIN/OCMS/OCMS_READ_MODEL_CONTRACT.md`
- `00_SYSTEM_BRAIN/OCMS/OCMS_READ_MODEL_MAPPING.md`
- `00_SYSTEM_BRAIN/OCMS/OCMS_READ_MODEL_EXAMPLES.md`
- `00_SYSTEM_BRAIN/002_DECISIONS/ADR_OCMS_READ_MODEL_CONTRACT_ADDENDUM.md`

Read Model materializes the conceptual frame as a **derived view** — not persistence.

This section does not alter §3–§13 definitions above.
