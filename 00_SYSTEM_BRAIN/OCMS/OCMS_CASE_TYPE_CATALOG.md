...
## 9. Read Model caseType field

Contract **`caseType`** must use catalog codes (`HO_SO`, `OPERATIONS`, `FINANCE`, …).

| P0 type | Typical `source` anchor |
|---------|-------------------------|
| OPERATIONS | TASK_ANCHORED |
| HO_SO | HO_SO_ANCHORED or MIXED |
| FINANCE | FINANCE_ANCHORED |

Mismatch (e.g. `caseType: HO_SO` without HO_SO relation) → `diagnostics.warnings`.

This section does not alter §1–§8 above.
