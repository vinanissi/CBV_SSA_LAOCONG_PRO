...
| **Identity** | Logical; **Case Key** — `OCMS_CASE_KEY_AUTHORITY.md` |
...
## 14. Read Model usage of CRM

`CaseReadModel` maps CRM pillars to contract fields:

| CRM pillar | Contract field |
|------------|----------------|
| Case / Case Key | `caseKey`, `title` |
| Case Type | `caseType` |
| Lifecycle | `lifecycle` |
| Result | `result` |
| Responsibility | `responsibility` |
| Work / Steps | `workItems[]` |
| Memory | `memorySummary` (summary only — not full Memory) |
| Relation | `relations[]` |
| Module Projection | `projections` |

Full Memory federation deferred to OCMS_04. Mutations remain on source runtimes.

Spec: `OCMS_READ_MODEL_CONTRACT.md`.
