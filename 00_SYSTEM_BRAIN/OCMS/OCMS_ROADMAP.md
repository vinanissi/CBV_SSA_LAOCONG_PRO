...
**Status:** Planning (OCMS_03 strip implemented — flag-gated)  
**ADR:** `ADR_OCMS_FOUNDATION.md`, `ADR_OCMS_FOUNDATION_CRM_ADDENDUM.md`, `ADR_OCMS_CASE_TYPE_RESULT_ADDENDUM.md`, `ADR_OCMS_CASE_LIFECYCLE_ADDENDUM.md`, `ADR_OCMS_CASE_RELATION_ADDENDUM.md`, `ADR_OCMS_READ_MODEL_CONTRACT_ADDENDUM.md`, `ADR_OCMS_CASE_STRIP_VISIBILITY_RULES.md`, `ADR_OCMS_CASE_STRIP_LAYOUT_AUTHORITY.md`
...
## Case-Centric Runtime Refactor (parallel branch)

Branch `phase/case-centric-runtime-refactor` extends OCMS with binding authority:

- `ADR_CASE_CENTRIC_RUNTIME.md`
- `00_SYSTEM_BRAIN/CASE/CASE_RUNTIME_AUTHORITY_V1.md`
- `CASE_ROADMAP.md` / `PHASE_CASE_REFACTOR_*`

OCMS phases 00–03E remain valid. Case refactor phases **02–04** implement read model / workspace / timeline without `CASE_MAIN` until `PHASE_CASE_REFACTOR_05_PERSISTENCE_DECISION`.
...
| **OCMS_01** `PHASE_OCMS_01_CASE_KEY_CONVENTION` | DOC-ONLY | Case Key rules, examples, mapping from TASK/HO_SO | None | None — **superseded for impl by OCMS_02D** |
...
| **OCMS_02A** `PHASE_OCMS_02A_CASE_STRIP_VISIBILITY_RULES` | DOC-ONLY | Case Strip visibility rules + ADR | None | Spec only |
| **OCMS_02B** `PHASE_OCMS_02B_CASE_STRIP_LAYOUT_AUTHORITY` | DOC-ONLY | Case Strip layout authority (Main Area placement) | None | Spec only |
| **OCMS_02C** `PHASE_OCMS_02C_CASE_DISCOVERY_AUTHORITY` | DOC-ONLY | Case Discovery authority (anchors, precedence, diagnostics) | None | Spec only |
| **OCMS_02D** `PHASE_OCMS_02D_CASE_KEY_AUTHORITY` | DOC-ONLY | Case Key identity authority (format, derive, diagnostics) | None | Spec only |
...
Phases **00A, 01, 01A, 01B, 01C, 02, 02A, 02B, 02C, 02D** can run as DOC-ONLY in parallel with Work Inbox runtime phases.

**Sequencing note:** Case Key identity is **authoritative in OCMS_02D** for implementation. `OCMS_01` row retained for history; OCMS_03+ MUST use `OCMS_CASE_KEY_AUTHORITY.md`.
...
## Phase history — OCMS_02A (2026-05-31)

| Metric | OCMS_02A |
|--------|----------|
| Visibility spec | `OCMS_CASE_STRIP_VISIBILITY_RULES.md` |
| ADR | `ADR_OCMS_CASE_STRIP_VISIBILITY_RULES.md` |
| Flag | `OCMS_CASE_STRIP_ENABLED` (required OCMS_03) |
| Production code diff | **None** |
| Work Inbox regression | **N/A** |

**OCMS_03 must implement:** visibility levels HIDDEN/MINIMAL/STANDARD/EXPANDED per spec.

---

## Phase history — OCMS_02B (2026-05-31)

| Metric | OCMS_02B |
|--------|----------|
| Layout authority | `OCMS_CASE_STRIP_LAYOUT_AUTHORITY.md` |
| ADR | `ADR_OCMS_CASE_STRIP_LAYOUT_AUTHORITY.md` |
| Placement | Main Area below CompactTaskHeader |
| Production code diff | **None** |
| Work Inbox regression | **N/A** |

**OCMS_03 must implement:** layout stack + height budgets; no Right Panel strip.

---

## Phase history — OCMS_02C (2026-05-31)

| Metric | OCMS_02C |
|--------|----------|
| Discovery authority | `OCMS_CASE_DISCOVERY_AUTHORITY.md` |
| ADR | `ADR_OCMS_CASE_DISCOVERY_AUTHORITY.md` |
| Precedence | MANUAL → HO_SO → FINANCE → ALERT → TASK |
| Production code diff | **None** |
| Work Inbox regression | **N/A** |

**OCMS_03 must implement:** discovery pipeline before derive; `discoveryCandidates[]` on multi-anchor tasks.

---

## Phase history — OCMS_02D (2026-05-31)

| Metric | OCMS_02D |
|--------|----------|
| Case Key authority | `OCMS_CASE_KEY_AUTHORITY.md` |
| ADR | `ADR_OCMS_CASE_KEY_AUTHORITY.md` |
| Supersedes | OCMS_01 for implementation identity rules |
| Production code diff | **None** |
| Work Inbox regression | **N/A** |

**OCMS_03 must implement:** namespace table key compose after discovery; `CASE_KEY_*` diagnostics.

---

## Phase history — OCMS_03 (2026-05-31)

| Metric | OCMS_03 |
|--------|---------|
| Runtime module | `apps/workboard/src/modules/ocms/` |
| Focus strip | `WorkInboxCaseContextStrip` below `CompactTaskHeader` |
| Flag | `VITE_OCMS_CASE_STRIP_ENABLED` (default OFF) |
| Schema / API / CASE_MAIN | **None** |
| Work Inbox regression | Right Panel unchanged; flag OFF = zero delta |
| Status | **GO_WITH_WARNINGS** — `RUNTIME_STATE: NOT_WIRED` |

**Ready for:** `PHASE_OCMS_04_FEDERATED_TIMELINE_READ`

---

## Phase history — OCMS_03A (2026-05-31)

| Metric | OCMS_03A |
|--------|----------|
| Mode | VERIFY (operator validation) |
| Suite | `runOcmsOperatorValidationChecks()` — 28 checks PASS |
| Code change | Validation suite only — no strip redesign |
| Live UAT | Skipped — NOT_WIRED |
| Status | **GO_WITH_WARNINGS** |

**Ready for:** staging operator UAT then `PHASE_OCMS_04_FEDERATED_TIMELINE_READ`

---

## Phase history — OCMS_03B (2026-05-31)

| Metric | OCMS_03B |
|--------|----------|
| Mode | VERIFY (real operator UAT) |
| Operator script | `OCMS_CASE_STRIP_UAT_OPERATOR_SCRIPT.md` |
| Telemetry | `ocmsStripUatTelemetry.ts` — sessionStorage only |
| Live sessions | **Not executed** — metrics N/A |
| Status | **GO_WITH_WARNINGS** |

**Follow-up:** Run O1–O8 in staging; paste `__OCMS_UAT_EXPORT__()` into evidence.

---

## Phase history — OCMS_03C (2026-05-31)

| Metric | OCMS_03C |
|--------|----------|
| Mode | VERIFY (operator evidence collection) |
| Evidence log | `OCMS_CASE_STRIP_OPERATOR_EVIDENCE_LOG.md` |
| Parser | `ocmsOperatorEvidenceParser.ts` |
| Real evidence | **NOT COLLECTED** — documented |
| OCMS_04 gate | **BLOCKED** until evidence rollup signed |
| Status | **GO_WITH_WARNINGS** |

---

## Phase history — OCMS_03D (2026-05-31)

| Metric | OCMS_03D |
|--------|----------|
| Mode | IMPLEMENT (FE strip enablement) |
| Flag | `VITE_OCMS_CASE_STRIP_ENABLED` — `.env.local` for dev |
| Resolver | `ocmsFocusTaskResolver.ts` |
| Strip fields | identity, discovery source, relations, diagnostics |
| Build | PASS |
| Status | **GO_WITH_WARNINGS** |

**Ready for:** operator evidence collection (03C) with visible strip

---

## Phase history — OCMS_03D Enrichment (2026-05-31)

| Metric | OCMS_03D Enrichment |
|--------|---------------------|
| Mode | IMPLEMENT (strip content enrichment) |
| Strip sections | identity, key, lifecycle, responsible, discovery, relations, diagnostics, visibility |
| Safe key | `TASK:id` format — no canonical key in DOM |
| Status | **GO_WITH_WARNINGS** |

---

## Phase history — OCMS_03E (2026-05-31)

| Metric | OCMS_03E |
|--------|----------|
| Mode | IMPLEMENT (operator UX polish) |
| Layout | 3-row compact context card + visibility-scoped fields |
| Relations | Action-oriented (`N việc liên quan`) |
| Diagnostics | Calm OK pill / warn pill |
| Status | **GO_WITH_WARNINGS** |

**Next:** Real operator UAT (03C gate) before OCMS_04
