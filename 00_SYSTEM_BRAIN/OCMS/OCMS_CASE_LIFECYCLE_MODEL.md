...
## 15. Lifecycle and Relation

| Lifecycle | Relation expectation |
|-----------|---------------------|
| NEW | Optional SOURCE (ALERT, TASK) |
| TRIAGE | Define PRIMARY + TARGET |
| ACTIVE | Maintain SECONDARY, DEPENDENCY |
| WAITING | DEPENDENCY on external entity |
| BLOCKED | BLOCKED_BY relation documented |
| REVIEW | EVIDENCE_REF complete for typed cases |
| CLOSED / ARCHIVED | Relations read-only |

Lifecycle **BLOCKED** correlates with **BLOCKED_BY** relation — not identical (lifecycle = phase; relation = which entity blocks).

Full spec: `OCMS_CASE_RELATION_MODEL.md` §12.

This section does not alter §1–§14 above.

---

## 16. Read Model lifecycle fields

Contract **`lifecycle`** object:

| Field | Maps from |
|-------|-----------|
| `code` | Inferred lifecycle state — not task status |
| `label` | Vietnamese operator label |
| `since` | Last transition timestamp (best effort) |
| `source` | DeriveSource |
| `confidence` | Inference quality |

Inference rules: `OCMS_READ_MODEL_MAPPING.md` §4.
