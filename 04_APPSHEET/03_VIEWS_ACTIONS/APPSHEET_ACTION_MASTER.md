...
- GAS routing: `99_APPSHEET_WEBHOOK.js` — sau `CMD:` strip, tra ACTION_REGISTRY rồi `withPendingFeedback()` + adapter; `withTaskFeedback()` chỉ là thin wrapper → `withPendingFeedback` + adapter TASK (cùng hành vi ⏳ / ✅ / ❌).
