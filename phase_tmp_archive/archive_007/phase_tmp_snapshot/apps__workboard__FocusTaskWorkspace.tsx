...
import { isOcmsCaseStripEnabled } from '@/modules/ocms/ocmsFeature';
...
export function FocusTaskWorkspace({
  ...
  const ocmsStripOn = isOcmsCaseStripEnabled();
  ...
  const { stripView, setCollapsed } = useCaseReadModel({
    enabled: ocmsStripOn,
    task: runtimeTask,
    focusItem: current,
    operationalBundle,
  ...
  return (
    ...
    >
      ...
      {ocmsStripOn && stripView ? (
        <WorkInboxCaseContextStrip view={stripView} onCollapse={() => setCollapsed(true)} />
      ) : null}
