...
/**
 * Physical tab name for logical sheet key (CONFIG_SHEET_REGISTRY or legacy HO_SO_V2.SHEETS).
 * @param {string} sheetKey — key of HO_SO_V2.SHEETS / TABLE_CODE
 * @returns {string}
 */
function hoSoV2ResolvePhysicalSheetName_(sheetKey) {
  var sk = String(sheetKey || '').trim();
  if (typeof CBV_Config_getSheetName === 'function') {
    try {
      var n = String(CBV_Config_getSheetName('HOSO', sk) || '').trim();
      if (n) return n;
    } catch (eCfg) {
      /* ignore */
    }
...
/**
 * HOSO data spreadsheet: isolated DB when CBV_Config_getDbId('HOSO') or Script Property CBV_HOSO_DB_ID set; else active.
 * @returns {GoogleAppsScript.Spreadsheet.Spreadsheet}
 */
function hoSoV2Spreadsheet_() {
...
function HoSoV2_bootstrapSheets() {
  ...
  for (i = 0; i < keys.length; i++) {
    var k = keys[i];
    var r = hoSoV2EnsureSheet_(k, k);
    out.push({
      sheet: hoSoV2ResolvePhysicalSheetName_(k),
      created: r.created,
