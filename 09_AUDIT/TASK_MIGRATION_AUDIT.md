...
**Date:** 2025-03-21  
**Scope:** 09_AUDIT/TASK_MIGRATION_PLAN.md, 05_GAS_RUNTIME/20_TASK_MIGRATION_HELPER.js
...
|---|-------|----------|----------|
| 1 | `ownerVal` used before assignment — `!ownerVal` check on line 265 runs before `ownerVal = _getCell(...)` on 266. Causes wrong unresolvedUserRefs count. | **CRITICAL** | 20_TASK_MIGRATION_HELPER.js:264-266 |
| 2 | RESULT column: when value contains embedded URL in text (e.g. "Done. See https://..."), no attachment created. DRAFT/SOP/REFERENCE use `_extractUrls`; RESULT only uses `_isUrl` for whole string. | Minor | 20_TASK_MIGRATION_HELPER.js:315 |
