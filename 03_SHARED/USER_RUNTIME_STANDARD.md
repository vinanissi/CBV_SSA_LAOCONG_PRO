...
| getUserByEmail(email) | 02_USER_SERVICE.gs | Lookup by email (case-insensitive) |
| assertValidUserId(userId, fieldName) | 02_USER_SERVICE.gs | User exists, not deleted (any status) |
| assertActiveUserId(userId, fieldName) | 02_USER_SERVICE.gs | User exists, not deleted, STATUS=ACTIVE |
| buildUserDisplayMap() | 02_USER_SERVICE.gs | Map ID -> display string for dropdowns |
| auditUserDirectory() | 90_BOOTSTRAP_AUDIT.gs | Full integrity audit |
...
8. **Validation reusable** — validateUserRecordForCreate, validateUserRecordForUpdate in 02_USER_VALIDATION.gs.
