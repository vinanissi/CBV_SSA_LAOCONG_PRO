/**
 * HO_SO — host-side CBV_CoreV2_dispatch (Module-led).
 * Ghi COMMAND_LOG / IDEMPOTENCY trên CORE DB; handler resolve từ globalThis host (HosoCommandHandler_handle).
 *
 * NOTE:
 * Dispatch nghiệp vụ không chạy trong CORE_RUNTIME_LIB vì library không thấy handler host.
 *
 * Dependencies: 021 command log, 022 idempotency, 050 permission, 070 registry read,
 * 005 bridge (cbvCoreV2* helpers), 00–02 locals.
 */
 ...
 */
function CBV_CoreV2_dispatch_host_(command) {
  ...
  if (!handler && String(norm.moduleCode || '').toUpperCase() === 'HOSO' && typeof g.HosoCommandHandler_handle === 'function') {
    handler = g.HosoCommandHandler_handle;
  }
...
function CBV_CoreV2_dispatch(command) {
  return CBV_CoreV2_dispatch_host_(command);
}
