 ...
 */
function cbvCoreV2GetSpreadsheet_local_() {
 ...
 * @returns {number}
 */
function cbvCoreV2SheetOutputWidth_local_(sheet) {
...
function cbvCoreV2ReadHeaderMap_local_(sheet) {
  var width = cbvCoreV2SheetOutputWidth_local_(sheet);
  if (width < 1) return {};
 ...
 */
function cbvCoreV2EnsureSheetWithHeaders_local_(sheetName, requiredHeaders) {
  var ss = cbvCoreV2GetSpreadsheet_local_();
 ...
 * @returns {{ sheet: GoogleAppsScript.Spreadsheet.Sheet, created: boolean, appendedHeaders: string[] }}
 */
function cbvCoreV2EnsureSheetWithHeadersOnSpreadsheet_local_(ss, sheetName, requiredHeaders) {
  var sheet = ss.getSheetByName(sheetName);
  var created = false;
  if (!sheet) {
    sheet = ss.insertSheet(sheetName);
 ...
 */
function cbvCoreV2AppendRowByHeaders_local_(sheet, valuesByHeader) {
  var width = cbvCoreV2SheetOutputWidth_local_(sheet);
 ...
 */
function cbvCoreV2EnsureCoreSheet_local_(sheetKey, headerKey) {
  var name = CBV_CORE_V2.SHEETS[sheetKey];
  var headers = CBV_CORE_V2.HEADERS[headerKey];
  return cbvCoreV2EnsureSheetWithHeaders_local_(name, headers);
}
 ...
 */
function cbvCoreV2FindFirstRowInColumn_local_(sheet, columnName, searchValue) {
  var map = cbvCoreV2ReadHeaderMap_local_(sheet);
 ...
 */
function cbvCoreV2UpdateRowByHeaders_local_(sheet, row, updatesByHeader) {
  var map = cbvCoreV2ReadHeaderMap_local_(sheet);
