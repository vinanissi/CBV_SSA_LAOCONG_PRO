...
function HoSoV2_Health_check() {
  ...
  var i;
  var hcCtx = { source: 'HEALTH' };

  for (i = 0; i < keys.length; i++) {
    var k = keys[i];
    var name =
      typeof hoSoV2ResolvePhysicalSheetName_ === 'function' ? hoSoV2ResolvePhysicalSheetName_(k, hcCtx) : HO_SO_V2.SHEETS[k];
    var sh = hoSoV2Spreadsheet_(hcCtx).getSheetByName(name);
    if (!sh) {
      ok = false;
  ...
  hoSoV2EnsureSheet_('HEALTH', 'HEALTH', hcCtx);
  var hsh =
    typeof HOSO_Config_getSheetWithFallback_ === 'function'
      ? HOSO_Config_getSheetWithFallback_('HEALTH', hcCtx)
      : hoSoV2GetSheet_('HEALTH');
...
/**
 * @returns {Object} same shape as HoSoV2_Health_check (menu/admin alias).
 */
function HosoService_healthCheck() {
  return HoSoV2_Health_check();
}
