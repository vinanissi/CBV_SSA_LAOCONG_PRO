...
function CBV_TestConsole_WebApp_getUrl_() {
  ...
  if (!base) return '';
  return base + (base.indexOf('?') >= 0 ? '&' : '?') + 'app=' + encodeURIComponent(CBV_TEST_CONSOLE_WEBAPP_APP_CODE_);
}
...
function CBV_TestConsole_WebApp_menuOpen() {
  ...
  var html =
    ...
    '<h3>CBV Test Console WebApp</h3><p>Open the standalone WebApp frontend:</p><p><a href="' +
    String(url).replace(/&/g, '&amp;').replace(/"/g, '&quot;') +
    '" target="_blank" rel="noopener">Open CBV Test Console WebApp</a></p><p style="color:#666;font-size:12px">The Sheet menu is only a launcher/bootstrap/health surface.</p></body></html>';
...
function CBV_TestConsole_WebApp_menuBootstrap() {
  var ui = SpreadsheetApp.getUi();
  try {
    ui.alert('CBV Test Console WebApp Bootstrap', JSON.stringify(cbvTestConsoleWebAppBootstrap(), null, 2), ui.ButtonSet.OK);
  } catch (e) {
...
function CBV_TestConsole_WebApp_menuHealthCheck() {
  var ui = SpreadsheetApp.getUi();
  try {
    ui.alert('CBV Test Console WebApp Health', JSON.stringify(cbvTestConsoleWebAppHealthCheck(), null, 2), ui.ButtonSet.OK);
  } catch (e) {
...
function CBV_TestConsole_WebApp_menuSelfTest() {
  var ui = SpreadsheetApp.getUi();
  try {
    ui.alert('CBV Test Console WebApp Self-Test', JSON.stringify(cbvTestConsoleWebAppSelfTest({ runSafeSuite: false }), null, 2), ui.ButtonSet.OK);
  } catch (e) {
