...
function CBV_TestConsole_WebApp_listReportsModel_(limit) {
  ...
  var out = [];
  var i;
  for (i = values.length - 1; i >= 0; i--) out.push(CBV_TestConsole_WebApp_reportRowToObject_(map, values[i], start + i));
...
function CBV_TestConsole_WebApp_findReportByTraceId_(traceId) {
  ...
  var i;
  for (i = 0; i < reports.length; i++) if (String(reports[i].traceId || '') === want) return reports[i];
  return null;
}
