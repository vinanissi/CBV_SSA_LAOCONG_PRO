...
function CBV_TestConsole_Timeline_append_(session, eventType, fromState, toState, message, payload) {
  ...
  return CBV_TestConsole_Runtime_appendByHeaders_(sh, CBV_TC_TIMELINE_HEADERS_, { CREATED_AT: CBV_TestConsole_Runtime_now_(), EVENT_ID: CBV_TestConsole_Runtime_uuid_('TCEVT'), SESSION_ID: String(s.sessionId || ''), TRACE_ID: String(s.traceId || ''), EVENT_TYPE: String(eventType || ''), FROM_STATE: String(fromState || ''), TO_STATE: String(toState || ''), ACTOR: CBV_TestConsole_Runtime_user_(), MESSAGE: String(message || ''), PAYLOAD_JSON: CBV_TestConsole_Runtime_safeJson_(payload || {}) });
}
...
function CBV_TestConsole_Timeline_listRecent_(limit, sessionId) {
  ...
  for (i = values.length - 1; i >= 0; i--) {
    var row = values[i];
    var obj = { createdAt: String(row[0] || ''), eventId: String(row[1] || ''), sessionId: String(row[2] || ''), traceId: String(row[3] || ''), eventType: String(row[4] || ''), fromState: String(row[5] || ''), toState: String(row[6] || ''), actor: String(row[7] || ''), message: String(row[8] || ''), payloadJson: String(row[9] || '{}') };
