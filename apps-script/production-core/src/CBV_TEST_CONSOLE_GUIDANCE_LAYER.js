...
function CBV_TestConsole_Guidance_buildForSession_(session, report) {
  ...
  return { state: state, blocked: blocked, status: status, severity: severity, warnings: warnings, nextActions: nextActions, memoryReminder: 'Append report, decision, trace, and AI handoff before claiming completion.', generatedAt: CBV_TestConsole_Runtime_now_() };
}

function CBV_TestConsole_Guidance_getCurrent_() {
  var session = CBV_TestConsole_Session_getActive_();
  if (!session) return { session: null, guidance: CBV_TestConsole_Guidance_buildForSession_({ state: 'IDLE' }, null) };
