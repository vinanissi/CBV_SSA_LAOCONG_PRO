...
function CBV_TestConsole_Session_appendLog_(session, action, note, guidance) {
  ...
  return CBV_TestConsole_Runtime_appendByHeaders_(sh, CBV_TC_SESSION_HEADERS_, { CREATED_AT: CBV_TestConsole_Runtime_now_(), SESSION_ID: String(s.sessionId || ''), TRACE_ID: String(s.traceId || ''), ACTION: String(action || ''), STATE: String(s.state || ''), SUITE_CODE: String(s.suiteCode || ''), RUN_BY: String(s.runBy || CBV_TestConsole_Runtime_user_()), LOCK_ID: String(s.lockId || ''), STATUS: String(s.status || 'ACTIVE'), GUIDANCE_JSON: CBV_TestConsole_Runtime_safeJson_(guidance || s.guidance || {}), NOTE: String(note || '') });
}
function CBV_TestConsole_Session_start_(payload) {
  var p = payload || {};
  var session = { sessionId: CBV_TestConsole_Runtime_uuid_('TCSES'), traceId: typeof CBV_TestConsole_newTraceId_ === 'function' ? CBV_TestConsole_newTraceId_() : CBV_TestConsole_Runtime_uuid_('TRACE'), state: 'SESSION_OPEN', suiteCode: String(p.suiteCode || '').trim().toUpperCase(), runBy: CBV_TestConsole_Runtime_user_(), startedAt: CBV_TestConsole_Runtime_now_(), status: 'ACTIVE', lockId: '', lastReport: null, guidance: {} };
...
function CBV_TestConsole_Session_update_(patch, action, note) {
  ...
  var p = patch || {};
  var k;
  for (k in p) if (Object.prototype.hasOwnProperty.call(p, k)) session[k] = p[k];
