...
var CBV_TC_RUNTIME_TRANSITIONS_ = {
  IDLE: ['SESSION_OPEN'],
  SESSION_OPEN: ['SUITE_SELECTED', 'LOCKED', 'CLOSED'],
  SUITE_SELECTED: ['LOCKED', 'RUNNING', 'CLOSED'],
  LOCKED: ['RUNNING', 'CLOSED'],
  RUNNING: ['REPORT_READY', 'RECOVERY_REQUIRED'],
  REPORT_READY: ['NEEDS_GUIDANCE', 'CLOSED', 'SUITE_SELECTED'],
  NEEDS_GUIDANCE: ['SUITE_SELECTED', 'RECOVERY_REQUIRED', 'CLOSED'],
  RECOVERY_REQUIRED: ['NEEDS_GUIDANCE', 'SUITE_SELECTED', 'CLOSED'],
  CLOSED: []
};
...
function CBV_TestConsole_State_isKnown_(state) {
  ...
  var i;
  for (i = 0; i < CBV_TC_RUNTIME_STATES_.length; i++) {
    if (CBV_TC_RUNTIME_STATES_[i] === s) return true;
  }
  return false;
}

function CBV_TestConsole_State_canTransition_(fromState, toState) {
  var from = String(fromState || 'IDLE').toUpperCase();
  var to = String(toState || '').toUpperCase();
  var list = CBV_TC_RUNTIME_TRANSITIONS_[from] || [];
  var i;
...
function CBV_TestConsole_State_transition_(toState, reason, payload) {
  ...
  var to = String(toState || '').toUpperCase();
  if (!CBV_TestConsole_State_isKnown_(to)) {
    return { ok: false, code: 'UNKNOWN_STATE', message: 'Unknown state: ' + to, session: session };
  }
  if (!CBV_TestConsole_State_canTransition_(from, to)) {
    return { ok: false, code: 'ILLEGAL_TRANSITION', message: 'Illegal transition ' + from + ' -> ' + to, session: session };
  }
  session.state = to;
