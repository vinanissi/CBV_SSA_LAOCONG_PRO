...
function CBV_TestConsole_Lock_appendLog_(lock, action, result, note) {
  ...
  return CBV_TestConsole_Runtime_appendByHeaders_(sh, CBV_TC_LOCK_LOG_HEADERS_, {
    CREATED_AT: CBV_TestConsole_Runtime_now_(),
    LOCK_ID: String(l.lockId || ''),
    ACTION: String(action || ''),
    SESSION_ID: String(l.sessionId || ''),
    TRACE_ID: String(l.traceId || ''),
    LOCKED_BY: String(l.lockedBy || CBV_TestConsole_Runtime_user_()),
    REASON: String(l.reason || ''),
    EXPIRES_AT: String(l.expiresAt || ''),
...
function CBV_TestConsole_Lock_acquire_(payload) {
  ...
  var lock = {
    lockId: CBV_TestConsole_Runtime_uuid_('TCLOCK'),
    sessionId: session.sessionId,
    traceId: session.traceId,
    lockedBy: CBV_TestConsole_Runtime_user_(),
    reason: String(p.reason || 'Guided runtime operation'),
    createdAt: CBV_TestConsole_Runtime_now_(),
...
function CBV_TestConsole_Recovery_buildPlan_(report, session) {
  ...
  return {
    recoveryId: CBV_TestConsole_Runtime_uuid_('TCREC'),
    sessionId: String(s.sessionId || ''),
    traceId: String(r.traceId || s.traceId || ''),
    status: 'OPEN',
    reason: String(r.status || '') === 'FAIL' ? 'REPORT_FAIL' : 'OPERATOR_REQUEST',
    steps: steps,
...
function CBV_TestConsole_Recovery_create_(payload) {
  ...
  if (typeof CBV_TestConsole_State_transition_ === 'function' && session.state !== 'RECOVERY_REQUIRED') {
    CBV_TestConsole_State_transition_('RECOVERY_REQUIRED', 'Recovery plan created', plan);
  }
