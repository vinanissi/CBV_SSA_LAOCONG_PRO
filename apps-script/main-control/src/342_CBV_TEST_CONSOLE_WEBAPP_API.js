...
function CBV_TestConsole_WebApp_response_(ok, code, message, data, errors, warnings) {
  return {
    ok: !!ok,
    code: String(code || (ok ? 'CBV_TEST_CONSOLE_WEBAPP_OK' : 'CBV_TEST_CONSOLE_WEBAPP_ERROR')),
...
function cbvTestConsoleWebAppRunSuite(payload) {
  try {
    ...
    if (!suiteCode) {
      return CBV_TestConsole_WebApp_response_(false, 'CBV_TEST_CONSOLE_WEBAPP_SUITE_REQUIRED', 'suiteCode is required', {}, ['suiteCode is required'], []);
    }

    if (typeof CBV_TestConsole_getSuite_ === 'function') {
      var suite = CBV_TestConsole_getSuite_(suiteCode);
      if (suite && suite.destructive) {
        return CBV_TestConsole_WebApp_response_(false, 'CBV_TEST_CONSOLE_WEBAPP_DESTRUCTIVE_BLOCKED', 'Destructive suites cannot run from WebApp FE.', { suite: suite }, ['DESTRUCTIVE_SUITE_BLOCKED'], []);
      }
    }
...
function cbvTestConsoleWebAppGetAiHandoff(payload) {
  try {
    ...
    if (!report) {
      return CBV_TestConsole_WebApp_response_(false, 'CBV_TEST_CONSOLE_WEBAPP_REPORT_REQUIRED', 'report or traceId is required', {}, ['report or traceId is required'], []);
    }
