/**
 * Work Inbox V3 — operational permission matrix.
 */
...
export function canWorkInboxOp(role: string | undefined, op: WorkInboxOperationalOp): boolean {
  const key = normalizeWorkInboxRole(role);
  return MATRIX[key][op] ?? false;
}

export function workInboxOpPermissions(role: string | undefined): Record<WorkInboxOperationalOp, boolean> {
  const key = normalizeWorkInboxRole(role);
  return { ...MATRIX[key] };
}

/** Map action codes to permission ops */
